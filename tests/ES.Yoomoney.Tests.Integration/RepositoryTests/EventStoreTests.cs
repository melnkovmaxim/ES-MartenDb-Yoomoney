...
public sealed class EventStoreTests(AppWebFactory factory)
{
        ...
        var tasks = Enumerable.Range(0, 5)
            .Select(
                _ => Task.Run(
                    async () =>
                    {
                        var bankAccount = await _eventStore.LoadAsync<BankAccountAggregate>(
                            accountId,
                            null,
    ...
    public async Task SequentialOperations_ShouldCompleteWithoutConflicts()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            var loadedBankAccount =
                await _eventStore.LoadAsync<BankAccountAggregate>(accountId, null, CancellationToken.None);
        ...
        // Assert
        var finalBankAccount =
            await _eventStore.LoadAsync<BankAccountAggregate>(accountId, null, CancellationToken.None);
        finalBankAccount.Should().NotBeNull();
        finalBankAccount!.Balance.Should().Be(1500); // Initial 1000 + 5 deposits of 100 each
        finalBankAccount.Version.Should()
            .Be(7); // Initial account (1) + first deposit (2) + 5 more deposits (3,4,5,6,7)
    }
    ...
    public async Task Store_TwoEventsWithSameId_ShouldThrow()
    {
        ...
        // Act
        var func = async () => await _eventStore.StoreAsync(bankAccount, CancellationToken.None);
